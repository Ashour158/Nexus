...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (DEV_PREVIEW_ENABLED) {
    ...
    return NextResponse.json(apiSuccess(hardenContactRecord(contact).customFields?.outboxEvents ?? []));
  }

  const res = await fetch(`${CONTACTS_SERVICE_URL}/api/v1/contacts/${params.id}/outbox`, {
    headers: {
      authorization: auth ?? '',
