...
export function createSamlStrategy(
  ...
  const verify = async (profile: Profile | null | undefined, done: VerifiedCallback) => {
    try {
  ...
  // @node-saml/passport-saml v5 requires separate signon/logout verify callbacks.
  // This service doesn't do anything different for SLO, so reuse the same lookup.
  return new SamlStrategy(
    {
      entryPoint: config.entryPoint,
      issuer: config.issuer,
      // @node-saml/passport-saml v5 renamed `cert` to `idpCert`.
      idpCert: config.cert,
