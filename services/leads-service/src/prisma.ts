import { PrismaClient } from '../../../node_modules/.prisma/leads-client/index.js';
...
const outbox = new OutboxPublisher('leads-service');
...
async function publishEvent(
  ...
  const payload = operation === 'delete'
    ? { id: record.id, action: 'DELETED' as const, source: 'leads-service' }
    : { ...record, source: 'leads-service' };
  try {
    await outbox.publish(prisma, TOPICS.LEADS, payload, {
      eventType,
      tenantId,
...
export function createLeadsPrisma() {
  const base = createPrismaClientWithReplicas(
    (url: string) => new PrismaClient({ datasources: { db: { url } } }),
    { connectionLimit: 3, poolTimeout: 10, writeUrl: process.env.LEADS_DATABASE_URL }
  );
...
export type LeadsPrisma = ReturnType<typeof createLeadsPrisma>;
