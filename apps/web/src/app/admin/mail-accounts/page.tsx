  ...
  return (
    <div className="space-y-6">
      ...
      {accountsQuery.isLoading ? (
        <div className="flex items-center justify-center gap-2 rounded-2xl border border-outline-variant bg-inverse-surface p-12 text-sm text-on-surface-variant">
          <Loader2 className="h-4 w-4 animate-spin" /> Loading accounts…
        </div>
      ) : accountsQuery.isError ? (
        <div className="flex items-center justify-center gap-2 rounded-2xl border border-outline-variant bg-inverse-surface p-12 text-sm text-error">
          <AlertTriangle className="h-4 w-4" /> Could not load mail accounts.
        </div>
      ) : accounts.length === 0 ? (
        <div className="rounded-2xl border border-outline-variant bg-inverse-surface p-12 text-center">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {accounts.map((acct) => (
            <div key={acct.id} className="flex flex-col rounded-2xl border border-outline-variant bg-inverse-surface p-5">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-outline-variant pt-3">
                ...
                <button
                  ...
                  className="ml-auto rounded-lg p-1.5 text-on-surface-variant hover:bg-surface-container-highest hover:text-white"
                  title="Edit"
                >
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-on-surface/50" onClick={onClose}>
      <form
        ...
        className="flex h-full w-full max-w-lg flex-col overflow-y-auto border-l border-outline-variant bg-inverse-surface shadow-2xl"
      >
        ...
        <div className="flex-1 space-y-5 px-6 py-5">
          ...
          {draft.provider === 'SMTP' ? (
            <div className="space-y-4 rounded-lg border border-outline-variant bg-inverse-surface/40 p-4">
