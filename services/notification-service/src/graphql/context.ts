import type { PrismaClient } from '../../../../node_modules/.prisma/notification-client/index.js';
...
function createLoaders(prisma: PrismaClient) {
  const notificationLoader = new DataLoader<string, any>(async (ids) => {
    const items = await prisma.notification.findMany({ where: { id: { in: [...ids] } } });
