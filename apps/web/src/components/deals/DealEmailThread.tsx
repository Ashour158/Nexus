...
export default function DealEmailThread({ dealId }: DealEmailThreadProps) {
  ...
  if (isLoading) return <p className="text-sm text-on-surface-variant">Loading deal emails...</p>;

  return (
    <section className="space-y-4">
      {threads.length === 0 ? <p className="text-sm text-on-surface-variant">No emails linked to this deal yet.</p> : null}
      {threads.map((thread) => (
        <article key={thread.id} className="rounded-lg border border-outline-variant p-4">
          <div className="mb-2 flex items-center justify-between gap-3">
            <h3 className="text-sm font-semibold text-on-surface">{thread.subject}</h3>
            <span className="text-xs text-on-surface-variant">{new Date(thread.sentAt).toLocaleString()}</span>
          </div>
          <p className="text-xs text-on-surface-variant">From: {thread.from}</p>
          <p className="mt-2 text-sm text-on-surface">{thread.snippet}</p>
          <div className="mt-3 space-y-2">
            ...
            <div className="flex justify-end">
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-md bg-inverse-surface px-3 py-1.5 text-sm font-medium text-white hover:bg-surface-container-high disabled:opacity-50"
              >
