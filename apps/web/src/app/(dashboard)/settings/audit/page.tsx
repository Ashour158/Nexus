...
export default function AuditLogPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-2 rounded-xl border border-outline-variant bg-surface p-3 md:grid-cols-4">
        ...
        <button
          ...
          className="self-end rounded bg-primary px-3 py-2 text-sm font-medium text-on-surface disabled:opacity-50"
        >
          Export CSV
        </button>
      </div>
      ...
      <div className="grid gap-2 md:grid-cols-4">
        <div className="rounded-lg border border-outline-variant bg-surface p-3">
          ...
          <p className="text-xl font-semibold text-on-surface">{summary.total}</p>
        </div>
        <div className="rounded-lg border border-outline-variant bg-surface p-3">
          <p className="text-xs text-on-surface-variant">Attention</p>
          <p className="text-sm text-on-surface">{summary.failed} failed</p>
          <p className="text-sm text-on-surface">{summary.blocked} blocked</p>
          <p className="text-sm text-on-surface">{summary.warnings} warning</p>
        </div>
        <div className="rounded-lg border border-outline-variant bg-surface p-3">
          <p className="text-xs text-on-surface-variant">Mode</p>
          <p className="text-sm text-on-surface">{summary.executedCount} executed</p>
          <p className="text-sm text-on-surface">{summary.dryRuns} dry-run</p>
        </div>
        <div className="rounded-lg border border-outline-variant bg-surface p-3">
          <p className="text-xs text-on-surface-variant">Top operation types</p>
          <p className="text-sm text-on-surface">{summary.topOperationTypes}</p>
        </div>
      </div>
                ...
                {filteredRecords.map((record) => (
                  <tr key={record.auditId}>
                    ...
                    <td className="px-3 py-3">
                      <p className="font-medium text-on-surface">{record.operationType}</p>
