/**
 * Currency conversion helper for planning forecast roll-ups.
 *
 * Deal events arrive with a raw per-deal currency. To make the event-driven
 * ForecastAggregate / DealForecastState roll-ups currency-correct, we convert
 * every deal amount to the tenant's base currency before it is aggregated.
 *
 * Rates + the base currency are fetched from finance-service and cached in
 * memory with a TTL so we do NOT call finance on every event.
 *
 ...
 * 1:1 conversion (baseAmount = amount) and log a rate-limited warning. A missing
 * rate must never lose or block an event.
 *
...
const CACHE_TTL_MS = Number(process.env.PLANNING_RATES_TTL_MS ?? 10 * 60 * 1000); // 10 min
const FETCH_TIMEOUT_MS = 5_000;
const DEFAULT_BASE_CURRENCY = (process.env.PLANNING_DEFAULT_BASE_CURRENCY ?? 'USD').toUpperCase();
...
function warnOnce(key: string, message: string, meta?: Record<string, unknown>): void {
  ...
  console.warn(
    JSON.stringify({ level: 'warn', service: 'planning-service', component: 'currency', message, ...meta })
  );
}
...
/** Shared singleton so the in-memory cache is reused across all events. */
