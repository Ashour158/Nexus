    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-on-surface">Automation Rules</h1>
      ...
      {/* Filters */}
      <div className="flex flex-wrap gap-3 rounded-2xl border border-outline-variant bg-surface p-4">
        <select
          value={moduleFilter}
          onChange={(e) => {
        ...
        {rulesQuery.isLoading || metaQuery.isLoading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-outline-variant">
              {rules.map((rule) => (
                <tr key={rule.id} className="hover:bg-surface/40">
                  ...
                  <td className="px-4 py-3 text-center">
                    <button
                      ...
                      className={`relative inline-flex h-5 w-9 items-center rounded-full transition ${
                        rule.isActive ? 'bg-success' : 'bg-surface'
                      }`}
                    >
                      <span
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-on-surface/50" onClick={onClose}>
      <form
        ...
        className="flex h-full w-full max-w-xl flex-col overflow-y-auto border-l border-outline-variant bg-surface shadow-2xl"
      >
          ...
          <div>
            ...
            <div className="space-y-3">
              {draft.actions.map((action, i) => (
                <div key={i} className="rounded-lg border border-outline-variant bg-surface/50 p-3">
        ...
        <div className="flex-1 px-6 py-5">
          {runsQuery.isLoading ? (
            ...
            <ul className="space-y-2">
              {runs.map((run) => (
                <li key={run.id} className="rounded-lg border border-outline-variant bg-surface/40 p-3">
