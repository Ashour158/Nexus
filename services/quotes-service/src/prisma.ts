...
const outbox = new OutboxPublisher('quotes-service');
...
async function publishEvent(
  ...
  const payload = operation === 'delete'
    ? { id: record.id, action: 'DELETED' as const, source: 'quotes-service' }
    : { ...record, source: 'quotes-service' };
  try {
    await outbox.publish(prisma, TOPICS.QUOTES, payload, {
      eventType,
      tenantId,
...
export type QuotesPrisma = ReturnType<typeof createQuotesPrisma>;
