...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (DEV_PREVIEW_ENABLED) {
    ...
    const audit = account.customFields?.auditTrail;
    return NextResponse.json(apiSuccess(Array.isArray(audit) ? audit : []));
  }

  const res = await fetch(`${CRM_SERVICE_URL}/api/v1/accounts/${params.id}/audit`, {
    headers: {
      authorization: auth ?? '',
