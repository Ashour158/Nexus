    ...
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-error bg-error-container/20 p-4">
        ...
        <div className="mt-4 max-w-md space-y-2 text-sm">
          ...
          <input value={confirm} onChange={(e) => setConfirm(e.target.value)} placeholder={data?.name ?? 'Tenant name'} className="w-full rounded border border-outline-variant bg-inverse-surface px-3 py-2" />
...
function Card({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl border border-outline-variant bg-inverse-surface p-4"><p className="text-xs text-on-surface-variant">{label}</p><p className="mt-1 text-2xl font-semibold">{value}</p></div>;
}
...
function Input({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
  return (
    <label className="text-sm">
      {label}
      <input type="number" value={value} onChange={(e) => onChange(Number(e.target.value || 0))} className="mt-1 w-full rounded border border-outline-variant bg-inverse-surface px-3 py-2" />
    </label>
  );
}
