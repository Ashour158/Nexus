  ...
  return (
    <main className="max-w-4xl space-y-4 p-4">
      ...
      <section className="space-y-3 rounded-xl border border-slate-200 bg-white p-4">
        ...
        <div className="flex flex-wrap gap-2"><button onClick={() => save.mutate('PUBLISHED')} className="rounded bg-indigo-600 px-3 py-2 text-sm text-white" disabled={!title || save.isPending}>Publish</button><button onClick={() => save.mutate('DRAFT')} className="rounded border border-slate-300 px-3 py-2 text-sm" disabled={!title || save.isPending}>Save draft</button><button onClick={async () => { const id = await prompt('Article ID to archive', 'Archive Article'); if (!id) return; archive.mutate(id); }} className="rounded border border-red-300 px-3 py-2 text-sm text-red-700">Archive</button></div>
        {PromptDialog}
      </section>
    </main>
  );
