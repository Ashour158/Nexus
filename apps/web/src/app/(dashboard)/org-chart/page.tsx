...
function OrgNode({ node, depth }: { node: OrgChartNode; depth: number }) {
  ...
  return (
    <div>
      <div
        className="flex items-center gap-3 rounded-lg border border-outline-variant bg-surface px-3 py-2.5 shadow-sm"
        style={{ marginInlineStart: depth * 24 }}
      >
        <button
          type="button"
          onClick={() => setOpen((o) => !o)}
          className={`text-on-surface-variant hover:text-on-surface ${hasReports ? '' : 'invisible'}`}
        ...
        <div className="min-w-0 flex-1">
          <div className="font-semibold text-on-surface">{node.name || 'Unnamed'}</div>
      ...
      {hasReports && open ? (
        <div className="mt-1.5 space-y-1.5 border-outline-variant">
          {reports.map((r) => (
  ...
  return (
    <div className="max-w-4xl p-6">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-on-surface">Org Chart</h1>
          <p className="mt-0.5 text-sm text-on-surface-variant">Reporting hierarchy across your organization.</p>
        </div>
      </div>
