...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (DEV_PREVIEW_ENABLED) {
    const deals = getDevPreviewState().deals.filter((deal) => {
      const contacts = Array.isArray(deal.contacts) ? deal.contacts : [];
      return deal.contactId === params.id || contacts.includes(params.id);
    });
    return NextResponse.json(apiSuccess(paginated(deals, req.nextUrl.searchParams)));
  }

  const qs = req.nextUrl.searchParams.toString();
  const res = await fetch(`${CONTACTS_SERVICE_URL}/api/v1/contacts/${params.id}/deals${qs ? `?${qs}` : ''}`, {
    headers: {
      authorization: auth ?? '',
