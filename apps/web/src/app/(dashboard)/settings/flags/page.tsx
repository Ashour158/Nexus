  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {flags.map((flag, idx) => (
            <div key={flag.name} className="rounded-xl border border-outline-variant bg-surface p-4">
              <div className="grid gap-3 lg:grid-cols-6">
                ...
                <input value={flag.tenants.join(',')} onChange={(e) => updateAt(idx, { tenants: e.target.value.split(',').map((v) => v.trim()).filter(Boolean) })} placeholder="tenants csv" className="rounded border border-outline-variant bg-surface px-2 py-1.5 text-xs" />
                <input value={flag.users} onChange={(e) => updateAt(idx, { users: e.target.value })} placeholder="user emails" className="rounded border border-outline-variant bg-surface px-2 py-1.5 text-xs" />
                <div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Create Feature Flag" size="lg">
          <div className="w-full rounded-xl border border-outline-variant bg-surface p-4">
            <h3 className="text-lg font-semibold">Create Feature Flag</h3>
            <div className="mt-3 grid gap-2">
              <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="FLAG_NAME" className="rounded border border-outline-variant bg-surface px-3 py-2 text-sm" />
              <input value={newDescription} onChange={(e) => setNewDescription(e.target.value)} placeholder="Description" className="rounded border border-outline-variant bg-surface px-3 py-2 text-sm" />
              <input value={newRollout} onChange={(e) => setNewRollout(e.target.value)} type="number" min={0} max={100} placeholder="Rollout %" className="rounded border border-outline-variant bg-surface px-3 py-2 text-sm" />
            </div>
