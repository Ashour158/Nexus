...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (DEV_PREVIEW_ENABLED) {
    ...
    return NextResponse.json(apiSuccess(hardened.customFields?.fieldHistory ?? []));
  }

  const res = await fetch(`${CONTACTS_SERVICE_URL}/api/v1/contacts/${params.id}/field-history`, {
    headers: {
      authorization: auth ?? '',
