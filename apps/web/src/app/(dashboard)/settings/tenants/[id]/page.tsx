...
export default function AdminTenantDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {banner ? <div className="rounded border border-outline-variant bg-surface px-3 py-2 text-sm text-outline">{banner}</div> : null}
      ...
      <section className="rounded-xl border border-outline-variant bg-surface p-4">
        ...
        <p className="text-sm text-outline">Plan: {data?.plan ?? ''} · Renewal: {data?.renewalDate ?? ''} · Usage limits editable below.</p>
      </section>
      ...
      <section className="rounded-xl border border-error bg-error-container/20 p-4">
        <h3 className="font-semibold text-error">Danger zone</h3>
        <div className="mt-3 flex flex-wrap gap-2">
          <button onClick={() => void patchTenant({ forceLogoutAll: true }, 'Tenant sessions revoked')} className="rounded border border-error px-3 py-1.5 text-sm text-error">Force logout all users</button>
          <button onClick={() => void patchTenant({ status: 'Suspended' }, 'Tenant suspended')} className="rounded border border-error px-3 py-1.5 text-sm text-error">Suspend tenant</button>
        </div>
        <div className="mt-4 max-w-md space-y-2 text-sm">
          <p className="text-outline">Type tenant name to enable hard delete:</p>
          <input value={confirm} onChange={(e) => setConfirm(e.target.value)} placeholder={data?.name ?? 'Tenant name'} className="w-full rounded border border-outline-variant bg-surface px-3 py-2" />
          <button onClick={() => void deleteTenant()} disabled={confirm !== (data?.name ?? '')} className="rounded border border-error px-3 py-1.5 text-error disabled:opacity-50">Delete all data</button>
        </div>
      </section>
      {ConfirmDialog}
...
function Card({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl border border-outline-variant bg-surface p-4"><p className="text-xs text-on-surface-variant">{label}</p><p className="mt-1 text-2xl font-semibold">{value}</p></div>;
}
...
function Input({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) {
  return (
    <label className="text-sm">
      {label}
      <input type="number" value={value} onChange={(e) => onChange(Number(e.target.value || 0))} className="mt-1 w-full rounded border border-outline-variant bg-surface px-3 py-2" />
    </label>
  );
}
