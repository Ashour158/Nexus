    ...
    async (r) => {
      r.get(
        ...
        async (request, reply) => {
          ...
          const where = { tenantId: jwt.tenantId, type: 'MEETING' as const, deletedAt: null as null };
      ...
      r.get(
        ...
        async (request, reply) => {
          ...
          const rows = await prisma.activity.findMany({
            where: { tenantId: jwt.tenantId, type: 'MEETING', deletedAt: null, startDate: { gte: now }, status: { in: ['PLANNED', 'IN_PROGRESS'] as const } },
      ...
      r.post(
        ...
        async (request, reply) => {
          ...
          const activity = await prisma.activity.findFirst({ where: { id, tenantId: jwt.tenantId, deletedAt: null } });
