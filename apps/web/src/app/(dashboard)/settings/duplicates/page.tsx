...
function MergePanel({ group, onClose, onMerged }: { group: DuplicateGroup; onClose: () => void; onMerged: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div className="flex max-h-[90vh] w-full max-w-5xl flex-col rounded-2xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b p-5"><h2 className="text-lg font-bold">Merge Duplicate {group.entityType}</h2><button onClick={onClose}><X className="h-5 w-5" /></button></div><div className="flex-1 overflow-auto p-5"><div className="mb-4 flex gap-2">{group.records.map((rec) => <button key={rec.recordId} onClick={() => setMasterId(rec.recordId)} className={`rounded-lg border px-3 py-1.5 text-sm ${masterId === rec.recordId ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-gray-300'}`}>{String(rec.data?.firstName || rec.data?.name || rec.recordId)}</button>)}</div><table className="w-full text-sm"><thead><tr><th className="px-3 py-2 text-start">Field</th>{group.records.map((r) => <th key={r.recordId} className="px-3 py-2 text-start">{String(r.data?.firstName || r.data?.name || r.recordId)}</th>)}<th className="bg-indigo-50 px-3 py-2 text-start">Merged</th></tr></thead><tbody>{fields.map((field) => (<tr key={field} className="border-t"><td className="px-3 py-2">{field}</td>{group.records.map((rec) => { const val = rec.data?.[field]; const selected = fieldSelections[field]?.sourceId === rec.recordId; return <td key={rec.recordId} className="px-3 py-2"><button onClick={() => setFieldSelections((s) => ({ ...s, [field]: { sourceId: rec.recordId, value: val } }))} className={`rounded px-2 py-1 ${selected ? 'bg-indigo-100' : 'hover:bg-gray-100'}`}>{Array.isArray(val) ? (val as string[]).join(', ') : String(val || '-')}</button></td>; })}<td className="bg-indigo-50 px-3 py-2">{String(fieldSelections[field]?.value ?? group.records.find((r) => r.recordId === masterId)?.data?.[field] ?? '-')}</td></tr>))}</tbody></table></div><div className="flex justify-end gap-3 border-t p-5"><button onClick={onClose} className="px-4 py-2 text-sm">Cancel</button><button onClick={() => merge.mutate()} className="rounded-lg bg-indigo-600 px-5 py-2 text-sm text-white">{merge.isPending ? 'Merging...' : 'Merge Records'}</button></div></div></div>;
}
...
export default function DuplicatesPage() {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-6">{merging && <MergePanel group={merging} onClose={() => setMerging(null)} onMerged={() => setMerging(null)} />}<div className="mb-6 flex items-center justify-between"><div><h1 className="text-2xl font-bold text-gray-900">Duplicate Records</h1></div><button onClick={() => scan.mutate()} className="flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white"><ScanLine className="h-4 w-4" />{scan.isPending ? 'Scanning...' : 'Scan Now'}</button></div><div className="mb-6 grid grid-cols-3 gap-4"><div className="rounded-xl border p-4"><p className="text-2xl font-bold text-amber-600">{statsData.pendingContacts ?? 0}</p></div><div className="rounded-xl border p-4"><p className="text-2xl font-bold text-amber-600">{statsData.pendingAccounts ?? 0}</p></div><div className="rounded-xl border p-4"><p className="text-2xl font-bold text-green-600">{statsData.mergedTotal ?? 0}</p></div></div><div className="mb-4 flex gap-2">{(['', 'contact', 'account'] as const).map((f) => <button key={f} onClick={() => setEntityFilter(f)} className={`rounded-lg px-3 py-1.5 text-sm ${entityFilter === f ? 'bg-indigo-600 text-white' : 'bg-gray-100'}`}>{f === '' ? 'All' : f}</button>)}</div>{isLoading ? <div className="h-20 animate-pulse rounded-xl bg-gray-100" /> : groups.length === 0 ? <div className="py-16 text-center text-gray-400"><CheckCircle className="mx-auto mb-3 h-12 w-12" /><p>No pending duplicates</p></div> : <div className="space-y-3">{groups.map((group) => <div key={group.id} className="rounded-xl border border-gray-200 bg-white p-4"><div className="flex items-center justify-between"><div className="text-sm font-semibold capitalize">{group.entityType} duplicate ({group.records.length})</div><div className="flex items-center gap-2"><button onClick={() => dismiss.mutate(group.id)} className="px-3 py-1.5 text-sm text-gray-500">Not duplicates</button><button onClick={() => setMerging(group)} className="flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-1.5 text-sm text-white"><GitMerge className="h-3.5 w-3.5" />Merge<ChevronRight className="h-3.5 w-3.5" /></button></div></div></div>)}</div>}</div>;}
