...
export function MeetingBookingWidget({ repUsername }: { repUsername: string }) {
  ...
  return (
    <section className="space-y-3 rounded-xl border border-slate-200 bg-white p-4">
      ...
      <div className="grid gap-2 md:grid-cols-2">{slots.map((slot) => <button key={slot} onClick={() => setSelected(slot)} className={`rounded border px-3 py-2 text-sm ${selected === slot ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-slate-300'}`}>{slot}</button>)}</div>
      ...
      <button onClick={() => { console.log('Meeting created and invite email sent.'); }} disabled={!selected || !name || !email} className="rounded bg-indigo-600 px-3 py-2 text-sm font-medium text-white disabled:opacity-50">Confirm booking</button>
    </section>
  );
}
