...
vi.mock('../prisma.js', () => ({
  createAccountsPrisma: vi.fn(() => mocks.prismaBase),
...
describe('accounts-service', () => {
  beforeAll(async () => {
    ...
    vi.stubEnv('ACCOUNTS_DATABASE_URL', 'postgresql://test:test@localhost:5432/accounts_test');
  ...
  it('health check returns healthy', async () => {
    ...
    expect(res.json()).toMatchObject({ status: 'healthy', service: 'accounts-service' });
  });
});
