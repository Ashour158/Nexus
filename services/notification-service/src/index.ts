...
import { startBlueprintConsumer } from './consumers/blueprint.consumer.js';
...
try {
  ...
  // Blueprint (playbook / stage-transition / SLA) user-facing events → in-app
  // notifications. Before this, blueprint.stage.notification /
  // blueprint.transition.notification (on TOPICS.NOTIFICATIONS, no handler) and
  // blueprint.sla.breached (on TOPICS.BLUEPRINT, no subscriber) reached no one.
  await startBlueprintConsumer({ inApp, log: app.log });
